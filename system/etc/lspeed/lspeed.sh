...
governorTuner() {
sendToLog "Starting Governor tuner"

sendToLog "Number of CPU cores: $real_cpu_cores"

n=0
while [ "$n" -le "$cpu_cores" ]; do

    n=$(( n + 1 ))
done

if [ "$1" = "1" ]; then
	sendToLog "Executing power saving profile"
elif [ "$1" = "2" ]; then
	sendToLog "Executing balanced profile"
elif [ "$1" = "3" ]; then
	sendToLog "Executing performance profile"
fi
